...
### 1. Dynamic (Dynamic Range Quantization)
...
### 2. Static (Integer Quantization with float fallback with UINT8 inference input type)
...
### 3. Convert-only (No optimization)
*  Default settings/ direct tensorflow to tflite model conversion is used based on this [official implementation] (https://github.com/tensorflow/models/blob/master/research/object_detection/colab_tutorials/eager_few_shot_od_training_tflite.ipynb) and the [documentation](https://www.tensorflow.org/lite/convert#converting_a_savedmodel_).
