...
class ObjectDetectorLite:
    def __init__(self, model_path, label_path):
        ...
        self.interpreter = Interpreter(model_path=model_path, num_threads= 1) #NUM_THREADS=1/ BASELINE WITH 32BIT RPI
        #self.interpreter = Interpreter(model_path=model_path)
        self.interpreter.allocate_tensors()
        self.input_details = self.interpreter.get_input_details()
        
        self.output_details = self.interpreter.get_output_details()
        print(self.input_details[0]['dtype'], end=' ')
    ...
    def detect(self, image, threshold=0.1,input_size=None, return_classes=False, my_fps=None):
        ...
        return (self._boxes_coordinates(image,
                                          ...
                                          np.squeeze(scores[0]),
                                          min_score_thresh=threshold, return_classes=return_classes), my_fps)
    ...
    def _boxes_coordinates(self,
                        ...
                        min_score_thresh=.5, return_classes=False):
        """
        ...
        for i in range(number_boxes):
            if scores is None or scores[i] > min_score_thresh:
                ...
                corres_classes.append(classes[i])

        if not return_classes:return np.array(detected_boxes), probabilities, categories
        else:return np.array(detected_boxes), probabilities, categories, corres_classes
